...
| 创意工坊 | 图片精修：传统图像处理算子（参数面板 schema 驱动）+ AI 叠加处理；蒙版可画；修图前后对比（Lightroom 式并排 Y/Y / 分割线拖动） |
...
| 模型管理 | 应用内模型下载（后台执行 + 进度条 + 取消 / 重试，官方 / 镜像源择优）；分类模型五档候选切换（x / l / m / s / n）；GPU 加速开关（⚙ 性能设置），发布版内置 DirectML 运行时（N / A / I 全系显卡下载即用） |
...
- **方式一（推荐）**：下载 Release 中的安装包并双击安装（最新稳定版 **v0.3.1**，约 460MB，内含 VCR 微服务与运行必需的全部 AI 模型；**v0.4.0-beta** 预发布已提供**无语义模型 MSI**，137MB——语义模型装后在「⚙ 性能设置 → 📥 模型下载」获取，改进内容见「十一、版本历史」）：
  ...
  - 历史版本（v0.3.0 / v0.2.0 / v0.1.0）见 [Releases](https://github.com/howforever2000/PhotoMangementSys/releases)
...
> 预发布版本：代码与标签已入库，安装包待构建。基线验证：`cargo test --lib` 108 passed / 0 failed；`vue-tsc --noEmit` 0 错误。

**地基：重构 + 性能（beta 重点）**

- **lib.rs 瘦身重构**：全部 Tauri 命令按领域迁出（devtools.rs 等模块），lib.rs 收敛至 458 行，行为零改动；修复 2 条从未能编译的 doctest
- **性能优化**：重 IO 命令异步化、锁粒度收敛、缩略图并行生成；日志组件升级（分级过滤 / 精确时间戳 / 轮转 / 监控）
- **识别服务治理**：修复 VCR 微服务被 PATH 上的 Store 占位符 python 拖死；修复 6 条长期失败的单元测试

**4.0 图像处理 · 初步能力（创意工坊）**

- 传统图像处理算子：算子注册表 + `/api/apply` 单入口，首批 5 个算子，参数面板 schema 驱动（FEAT-066）；区域直方图均衡化小组件（FEAT-063）
- AI 叠加处理：多次处理可叠加成链，「原图」跟随最近基底
- **修图前后对比**（FEAT-068）：Lightroom 式左右并排（Y/Y）与单画布分割对比（分割线可拖动）；灯箱「编辑」一键直达创意工坊（FEAT-065）

**检索增强（3.x 主线收官补完，FEAT-067）**

- 以图搜图（复用图像向量）；描述向量三路 RRF；人物真名嵌入 + 改名/合并增量重算；扫描入库补齐地点（GPS → geo_index 离线反查）

**修复**

- **语义检索可用性修复**（BUG-2026-0920-009 / 008、BUG-2026-0921-003）：语义提示横幅脱链、退避分级可自愈、档位失配显性化——修复「检索命中 417 条却一条都不渲染」这类静默失效
- **面板主题变量收尾**（BUG-2026-0921-005）：全局扫描面板 / 扫描面板 / 相册信息头 / 确认对话框的浅色硬编码与 `var(--color-text)` 组合全部改主题变量（main.css 新增软色板，含深色对应值），修复深色模式下白底卡片配浅色文字不可读
- **主题文字对比度两层模型**（BUG-2026-0919-002 / 004）：文字色不再只跟浅/深模式走——卡片内文字跟随模式（底色同源，永远对比充足）；直接落在页面背景上的标题改用 `--color-on-bg` 变量，按实际背景等效色（纯色 = 自身 / 渐变 = 两端均值 / 背景图 = 32×32 采样均色×透明度混合）做 WCAG 对比度计算，偏好色 ≥4.5:1 原样保留，否则黑/白取对比更高一端；背景图场景叠加随文字深浅自动反转的细阴影。修复「深色模式 × 浅色背景/背景图时文字与背景融为一体」及 v1 整体翻转导致的「卡片深底配深字」回归。
- **登录页玻璃态改版**（BUG-2026-0919-001）：遮罩两层渐变 alpha 从 0.55~0.9 减至 0.10~0.52，登录卡片改半透明深底 + blur(14px) + 饱和度补偿，透出相册封面氛围。
- **人物注册表按相册过滤**（BUG-2026-0919-003）：相册详情页扫描面板的人物注脚表此前读全局 `list_persons`，其他相册的人物与全局脸数会混入；新增 `list_persons_in_album` 命令（faces 全表 → 复用 `p_is_under` 归属解析 → 按人物聚合去重照片数，降序），挂载即加载。
- **发布版内置 DirectML GPU 运行时**（BUG-2026-0919-005）：VCR 微服务打包依赖从 `onnxruntime` 切到 `onnxruntime-directml`（同名平替，多出 DmlExecutionProvider，走 DirectX 12），NVIDIA / AMD / Intel 全系显卡**下载即用**，无需用户自行 pip 安装；无 DX12 环境自动回退 CPU；版本仍 pin 1.24.4 规避 1.25+ 的 CLIP fp16 CPU 慢 2.9 倍坑（BUG-2026-0921-002）。实测 DML 会话对人物检测/人脸生效。
